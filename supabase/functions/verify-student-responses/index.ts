...
Deno.serve(serve(FN, async (req, rid) => {
  ...
  if (!pieceId || corrections.length === 0) return e(FN, 400, "pieceId and corrections[] required", { requestId: rid, code: "invalid_input" });
  if (corrections.length > 500) return e(FN, 400, "too many corrections", { requestId: rid, code: "too_many" });
  ...
  const rows = corrections
    .filter((c) => (typeof c?.blockId === "string" || typeof c?.segmentId === "string") && typeof c?.correctedText === "string")
    .map((c) => ({
  ...
  if (rows.length === 0) return e(FN, 400, "no valid corrections", { requestId: rid, code: "invalid_input" });

  const { error: insErr } = await admin.from("verification_corrections").insert(rows);
  if (insErr) return e(FN, 500, "Failed to store corrections", { requestId: rid, code: "insert_failed", cause: insErr });

  await advanceStage(admin, { pieceId, to: "responses_verified" });
  await logPieceEvent(admin, { pieceId, userId, event: "verification_completed", metadata: { count: rows.length } });
