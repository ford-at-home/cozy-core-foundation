...
Deno.serve(serve(FN, async (req, rid) => {
  ...
  const { data: piece } = await admin.from("pieces").select("id, user_id, workflow_stage").eq("id", pieceId).maybeSingle();
  ...
  const { data: run, error: runErr } = await admin.from("agent_runs").insert({
    user_id: userId, piece_id: pieceId, kind: "final_docx", status: "requested",
    idempotency_key: idempotencyKey, input: { pieceId, kind: "final_docx" },
  }).select("id").single();
  if (runErr) return e(FN, 500, "Failed to create run", { requestId: rid, code: "insert_failed", cause: runErr });

  const res = await reserveCreditsForRun(admin, { userId, runId: run.id, amount: COST, reason: "final DOCX" });
  if (!res.ok) {
  ...
  const { data: artifact } = await admin.from("final_artifacts").insert({
    piece_id: pieceId, user_id: userId, run_id: run.id, kind: "docx", status: "pending",
  }).select("id").single();
