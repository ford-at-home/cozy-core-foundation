...
Deno.serve(serve(FN, async (req, rid) => {
  ...
  const { data: packet } = await admin
    .from("packets").select("id, user_id, piece_id, version").eq("id", packetId).maybeSingle();
  ...
  const { count } = await admin
    ...
    .eq("packet_id", packetId).eq("status", "approved");
  if ((count ?? 0) < 1) {
    return e(FN, 422, "at least one approved follow-up question is required", { requestId: rid, code: "no_approved_questions" });
  }

  const idempotencyKey = `followup:${userId}:${requestId}`;
  const { data: existing } = await admin
    .from("agent_runs").select("id").eq("idempotency_key", idempotencyKey).maybeSingle();
  if (existing) return j({ runId: existing.id, idempotent: true }, 202, rid);

  if (creditsEnforced()) {
    const balance = await getBalance(admin, userId);
    if (balance < COST) return e(FN, 402, "Not enough credits", { requestId: rid, code: "insufficient_credits", details: { balance, required: COST } });
  }

  const { data: run, error: runErr } = await admin
    .from("agent_runs").insert({
      user_id: userId, piece_id: packet.piece_id, kind: "followup_research",
      status: "requested", idempotency_key: idempotencyKey,
  ...
  const res = await reserveCreditsForRun(admin, {
    userId, runId: run.id, amount: COST, reason: "follow-up research",
  });
  if (!res.ok) {
