...
Deno.serve(serve(FN, async (req, rid) => {
  ...
  if (!packetId || !transcript) return e(FN, 400, "packetId and transcript required", { requestId: rid, code: "invalid_input" });
  if (transcript.length > 20000) return e(FN, 400, "transcript too long", { requestId: rid, code: "too_long" });

  const { data: packet } = await admin.from("packets").select("id, user_id").eq("id", packetId).maybeSingle();
  if (!packet || packet.user_id !== userId) return e(FN, 404, "Packet not found", { requestId: rid, code: "not_found" });

  const { data: seg, error: err } = await admin.from("dictation_segments").insert({
    return_id: returnId, packet_id: packetId, user_id: userId, transcript,
    resolved_target: resolvedTarget, segment_order: segmentOrder, storage_path: storagePath,
  }).select("id").single();
  if (err) return e(FN, 500, "Failed to store dictation", { requestId: rid, code: "insert_failed", cause: err });
