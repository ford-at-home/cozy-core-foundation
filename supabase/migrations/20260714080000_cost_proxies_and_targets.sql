...
-- Revives the schema half of PR #4 (cursor/cost-calibration-ui-065e) on current main.
-- UI that reads these objects lands in a follow-up Cursor PR after this migration
-- is applied and generated types are regenerated (WI-0010).

-- =====================================================================
-- 1. workflow_cost_targets — planning budgets (editable in-DB anytime)
-- =====================================================================
CREATE TABLE IF NOT EXISTS public.workflow_cost_targets (
...
CREATE INDEX IF NOT EXISTS workflow_cost_targets_lookup_idx
...
DROP POLICY IF EXISTS "Anyone signed in can read cost targets" ON public.workflow_cost_targets;
...
INSERT INTO public.workflow_cost_targets (unit, target_usd, notes, effective_from)
VALUES
  (
    'full_piece',
    2.00000000,
    'Planning target: research → proposal → draft → revision for one piece',
    '2026-07-12 00:00:00+00'
  ),
  (
    'research_run',
    3.50000000,
    'Planning target: Parallel ultra-fast deep research',
    '2026-07-12 00:00:00+00'
  ),
  (
    'cursor_run',
    0.75000000,
    'Planning target: one Cursor Cloud Agent session (calibrate from invoices)',
    '2026-07-12 00:00:00+00'
  ),
  (
    'proposal_run',
    1.00000000,
    'Planning target: compose/resynth agent run with typical research',
    '2026-07-12 00:00:00+00'
  )
ON CONFLICT (unit, effective_from) DO NOTHING;
...
-- CREATE OR REPLACE preserves existing EXECUTE grants/revokes on this OID.
...
REVOKE EXECUTE ON FUNCTION public.tg_agent_runs_cost_proxies() FROM PUBLIC, anon, authenticated;
...
-- Backfill proxies for existing runs (idempotent on replay).
DO $$
DECLARE
  _rid uuid;
BEGIN
