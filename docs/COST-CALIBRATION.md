...
Compose tracks cost at **application boundaries** (Parallel tasks, Cursor agent
runs, Lovable/OpenAI gateway calls). Cursor's internal agent loops are not
exposed by API v0 — calibrate placeholders from invoices instead of expecting
per-turn token truth.
...
| Budget badge | `workflow_cost_targets` vs session total | Planning comparison |
...
### 2. Export tracked totals from Compose
...
CROSS JOIN LATERAL (
  ...
  WHERE unit = 'full_piece' AND effective_from <= now()
...
- Gateway pricing seeds: `supabase/migrations/20260712110000_*.sql`
- Proxy + targets migration: `supabase/migrations/20260712113000_*.sql`
