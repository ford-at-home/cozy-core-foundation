...
edits anything. Compose is a TanStack Start + Supabase app with unusual
boundaries (client UPDATE revoked on core tables, a vendored content contract,
a print/PDF sync rule). Most past agent mistakes were wrong-layer mistakes:
...
3. For operations questions: `docs/RUNBOOK.md`. For product/orchestration
   design rationale: `docs/cloud-agents-architecture-plan.md`.
...
- Adding a `tailwind.config.js` — Tailwind v4 here is configured in
  `src/styles.css`.
- Writing a client-side UPDATE to `pieces`/`agent_runs` — revoked by
