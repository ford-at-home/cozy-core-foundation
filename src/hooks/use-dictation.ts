// Shared dictation hook: microphone recording via Web Audio (PCM → WAV so
// the upload is a complete decodable file on every browser — Safari MP4
// fragments won't) plus transcription through /api/transcribe. Extracted
// from the profile page so packet returns reuse the exact same path.
...
function getUserMediaError(err: unknown): DictationError {
...
function transcribeHttpError(status: number, bodyError?: string): DictationError {
  if (status === 402) {
    return {
  ...
  if (status >= 500) {
    return {
      ...
      hint: bodyError ? `${bodyError} — press Retry in a moment.` : "Press Retry in a moment.",
      retryable: true,
    };
  }
  return {
    message: bodyError ?? `Transcription failed (${status}).`,
...
export function useDictation(onTranscript: (text: string) => void) {
  ...
  async function transcribeBlob(blob: Blob) {
    ...
    try {
      ...
      if (!res.ok) {
        setError(transcribeHttpError(res.status, body.error));
        return;
      }
  ...
  function retry() {
    if (lastBlob) void transcribeBlob(lastBlob);
  }
  ...
  return { recording, transcribing, error, lastBlob, start, stop, retry };
}
